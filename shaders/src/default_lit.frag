...
vec3 CalcDirLight(vec3 normal, vec3 viewDir){
	...
	// combine results
	vec3 ambient = vec3(sceneData.lightAmbient) * material.diffuse;
	vec3 diffuse = vec3(sceneData.lightDiffuse) * diff * material.diffuse;
	vec3 specular = vec3(sceneData.lightSpecular) * spec * material.specular;
